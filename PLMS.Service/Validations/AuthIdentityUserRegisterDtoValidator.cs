...
namespace PLMS.Service.Validations
{
    public class AuthIdentityUserRegisterDtoValidator : AbstractValidator<AuthIdentityUserRegisterDto>
    {
        public AuthIdentityUserRegisterDtoValidator()
        {
            ...
            RuleFor(x => x.PasswordMatch).NotNull().WithMessage("{PropertyName} is required.").Equal(x => x.Password).WithMessage("Passwords must be same").NotEmpty().WithMessage("{PropertyName} cannot be empty");
        }
    }
}
